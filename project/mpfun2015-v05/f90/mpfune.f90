...
subroutine mpbesselj (anu, t, z, mpnw)
...
if (mpnw < 4 .or. anu(0) < mpnw + 4 .or. anu(0) < abs (anu(2)) + 4 .or. &
  ...
  write (mpldb, 1)
1 format ('*** MPBESSELJ: uninitialized or inadequately sized arrays')
  call mpabrt (99)
endif
...
if (anu(2) <= 0.d0 .or. anu(3) > 0.d0 .or. &
  ...
  write (6, 2) anumx, anu
2 format ('*** MPBESSELJ: First argument must be >= 0 and <=', &
...
if (t(3) < 0.d0 .or. t(3) == 0.d0 .and. t(4) < dasy * (mpnw - 2)) then
  ...
  write (6, 3)
3 format ('*** MPBESSELJ: loop overflow 1')
  call mpabrt (66)
...
100 continue
  ...
  write (6, 4)
4 format ('*** MPBESSELJ: loop overflow 2')
  call mpabrt (66)
...
write (6, 3) 1, itrmx
3 format ('*** MPGAMMA: iteration limit execeeded',2i10)
call mpabrt (67)
...
subroutine mpincgamma (s, z, g, mpnw)
...
if (mpnw < 4 .or. s(0) < mpnw + 4 .or. s(0) < abs (s(2)) + 4 .or. &
  ...
  write (mpldb, 1)
1 format ('*** MPINCGAM: uninitialized or inadequately sized arrays')
  call mpabrt (99)
endif
...
2 format ('*** GAMMAINC: iteration limit exceeded:',2i10)
  call mpabrt (101)
...
return
end subroutine mpincgamma

subroutine mpzeta (ss, zz, mpnw)
...
if (mpnw < 4 .or. ss(0) < mpnw + 4 .or. ss(0) < abs (ss(2)) + 4 .or. &
  zz(0) < mpnw + 6) then
  write (mpldb, 1)
1 format ('*** MPZETA: uninitialized or inadequately sized arrays')
  call mpabrt (99)
endif
...
3 format ('*** MPZETA: iteration limit exceeded',2i10)
  call mpabrt (101)
endif  
...
return
end subroutine mpzeta

subroutine mpzetaem (nb1, nb2, berne, s, z, mpnw)
...
if (mpnw < 4 .or. s(0) < mpnw + 4 .or. s(0) < abs (s(2)) + 4 .or. &
  z(0) < mpnw + 6) then
  write (mpldb, 1)
1 format ('*** MPZETAEM: uninitialized or inadequately sized arrays')
  call mpabrt (99)
endif
...
if (s(2) == 1.d0 .and. s(3) == 0.d0 .and. s(4) == 1.d0) then
  write (mpldb, 2)
2 format ('*** MPZETAEM: argument is 1')
  call mpabrt (63)
endif
...
3 format ('*** MPZETAEM: Bernoulli coefficient array must be initialized'/ &
...
4 format ('*** MPZETAEM: iteration limit exceeded',2i10)
  call mpabrt (101)
endif  
